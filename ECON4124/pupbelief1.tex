...
\begin{tikzpicture}[
    ...
    \draw[black, green arrow] (root) -- (qg_mid) node[midway, above] {$UP$};
    ...
    \draw[red, green arrow] (qg_top) -- (leaf1) node[midway, above] {$C$};
    \draw[red] (qg_top) -- (leaf2) node[midway, above right] {$NC$};
    
    \draw[red, green arrow] (qg_mid) -- (leaf3) node[midway, above] {$C$};
    \draw[red] (qg_mid) -- (leaf4) node[midway, above right] {$NC$};

    % Nodes & Labels (Beliefs in green: 0 and 1)
    \node[dot=black, label=left:Student] at (root) {};
    \node[dot=red, label={[red]above left:Prof}, label={[gamegreen]above right:1}] at (qg_top) {};
    \node[dot=red, label={[gamegreen]above right:0}] at (qg_mid) {};
