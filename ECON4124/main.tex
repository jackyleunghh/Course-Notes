...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \[
        \begin{cases}
            (No\;Goal) \; u_D(\leftarrow_M, \leftarrow_D) = u_D(\rightarrow_M, \rightarrow_M) = 1 \\
...
\begin{itemize}
    ...
    \]
    \begin{itemize}
        \item \textit{Proof.} If: Suppose \(a^*_i \in B_i(\mathbf{a}^*_{-i})\) for every player \(i\), then \(u_i(a_i^*,\mathbf{a}_{-i}^*) \ge u_i(a_i,\mathbf{a}_{-1}^*)\) for all \(a_i \in A_i\), so by definition \(\mathbf{a}^*\) is a Nash equilibrium.

        Only if: Suppose \(\mathbf{a}^*\) is a Nash equilibrium, then \(u_i(a_i^*,\mathbf{a}_{-i}^*) \ge u_i(a_i,\mathbf{a}_{-1}^*)\) for all \(a_i \in A_i\), so by definition \(a^*_i \in B_i(\mathbf{a}^*_{-i})\) for every player \(i\).
    \end{itemize}
...
\begin{itemize}
    ...
    \item Intuitively, we should know \(V \succ 0.9V + 0.1H \succ H\). But \(0.9V + 0.1H \succ \text{or} \prec P\)?
    ...
    \end{itemize}
    \item \textbf{Example 2.6. Penalty Shootout with Mixed Strategy.} By the tick method, we can find that the game has no pure strategy Nash equilibrium. How about mixed strategy?
    \begin{itemize}
    ...
    \item \textbf{Example 2.7. Volunteer's Dilemma.} \(k\) students are in the classroom, where the professor has just said something confusing. Each of the student decides to raise their hand and ask for clarification or not, but doing so is costly:
    \[
    u_i =
    \begin{cases}
...
\end{table}
    ...
    \item \textbf{Example 2.8. Guess \(\frac{2}{3}\) of the Average.} A group of \(n\) players are guessing a real number \(a_i \in [0,100]\). Their goal is to guess as close as the \(\frac{2}{3}\) of the average guess as possible, so:
    \[
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Adam can deviate to \(A_A(\emptyset) = R\) instead of \(H\): \(U_A(R,R) = 1 > 0 = U_A(H,R)\).
        \item Eve can deviate to \(A_E(H_A) = H\) instead of \(R\): \(U_E(H,H) = 1 > 0 = U_E(H,R)\).
    \end{itemize}
    ...
    \begin{enumerate}
        ...
        \item Knowing this, Player 1 must maximize. The SPE is \(x = 100\): Player 1 keeps all the money.
    \end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Utility depends both the action profile and the type profile. For example, \(u_E((H,H)|(n,n)) = 1\), \(u_E((H,H)|(n,m)) = 0\).
    ...
    \]
    \begin{itemize}
        \item \textit{Remark.} For simplicity, suppose there is no ties.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item On the other hand, they can have any \textbf{off-equilibrium belief} for the ``college" path and then take any hypothetical best response.
        ...
        \begin{itemize}
            ...
            \item It depends on the firm's belief for a college-attending Alice. Let \(p = \mu(Smart, College) = \mathbb{P}(Smart \mid College)\).
    ...
    \begin{itemize}
        ...
        \item We know that Average Alice must not go to college. However, the pooling equilibrium requires that \(p \le \frac{1}{3}\), that is, an Alice who went to college is unlikely to be Smart!
    ...
    \end{itemize}
    \item A takeaway: If a signaling mechanism (e.g. exam, job interview) is too weak to differentiate between strong and weak candidates (i.e. it is not too costly for weak candidates), it is possible that it fails to reach a separating equilibrium.

    \item \textbf{Example 5.4. Bayesian Persuasion.} The Governor calls the President and claims that there's an epidemic in his province, asking for a relief fund.
    ...
    The Governor only wants the fund regardless of the truth: \(u_G(Y) = 1\) and \(u_G(N) = 0\). The President wants it to match: \(u_P(y,Y) = u_P(n,N) = 1\) and \(u_P(y,N) = u_P(n,Y) = 0\).

    \begin{itemize}
        \item If the state is more likely to be \(n\) (\(q<0.5\)), it is easy to verify that the President's best response is to always reject (\(N\)) given a consistent belief.
        ...
        \item Suppose the Governor hires an inspector with ability \((\alpha, \beta)\), where \(\alpha\) is the probability of \(\hat{y}\) if there is an epidemic, and \(\beta\) is the probability of \(\hat{y}\) if there is not, and furthermore assume \(\alpha \ge \beta\).
...
\begin{tabular}{llcc}
&&
\multicolumn{2}{c}{Inspector}  \\&& 
\(\hat{y}\) & \(\hat{n}\)
\\ \cline{3-4} 
\multirow{2}{*}{Nature} & 
\multicolumn{1}{c|}{\(y\)} & 
\multicolumn{1}{c|}{\(\alpha\)} &
\multicolumn{1}{c|}{\(1-\alpha\)} 
\\ \cline{3-4} & 
\multicolumn{1}{c|}{\(n\)} & 
\multicolumn{1}{c|}{\(\beta\)} &
\multicolumn{1}{c|}{\(1-\beta\)} 
\\ \cline{3-4} 
\end{tabular}
\end{center}
\end{table}
