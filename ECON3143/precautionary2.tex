...
\begin{tikzpicture}[scale=1.5]
    ...
    % Markers
    \coordinate (L) at (0.55, 4);
    \fill[black] (L) circle (1.5pt);
    \draw[cyan, dashed] (L) -- (0.55, 0) node[below] {$C_{t+1}^L$};
    \draw[cyan, dashed] (L) -- (0, 4) node[left] {$u'(C_{t+1}^L)$};

    \coordinate (H) at (3.2, 0.7);
    \fill[black] (H) circle (1.5pt);
    \draw[red, dashed] (H) -- (3.2, 0) node[below] {$C_{t+1}^H$};
    \draw[red, dashed] (H) -- (0, 0.7) node[left] {$u'(C_{t+1}^H)$};

    \coordinate (M) at (1.875, 2.35);
    \fill[black] (M) circle (1.5pt);
    \draw[black, dashed] (M) -- (1.875, 0) node[below] {$\mathbb{E}(C_{t+1})$};
    \draw[black, dashed] (M) -- (0, 2.35) node[left] {$\mathbb{E}(u'(C_{t+1}))$};
