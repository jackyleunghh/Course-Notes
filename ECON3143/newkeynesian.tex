...
% Point projections & labels
\draw[dashline] (\xr+\yp0, \yb+\yp0) -- (\xr+\yp0, \yb) node[below] {$Y^f_{0,t} = Y_{0,t}$};
...
\draw[curve] (\xr+\yp0, \ym) -- (\xr+\yp0, \ym+4.5) node[above right] {$AS^f$};

\draw[curve] (\xr+\yp0-2.2, \ym+\p0-1.8) -- (\xr+\yp0+2.2, \ym+4.5) node[right] {$AS$};

% AD Curve (Centered strictly on intersection)
\draw[curve] (\xr+\yp0-2.2, \ym+\p0+1.8) -- (\xr+\yp0+2.2, \ym+\p0-1.8) node[right] {$AD$}; 

% Point projections & labels
\draw[dashline] (\xr+\yp0, \ym+\p0) -- (\xr, \ym+\p0) node[left] {$\bar{P}_{0,t} = P_{0,t}$};
